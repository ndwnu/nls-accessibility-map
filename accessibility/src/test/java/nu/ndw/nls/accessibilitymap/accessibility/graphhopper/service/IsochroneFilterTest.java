...
class IsochroneFilterTest {
    ...
    void isWithinBoundingBox(Boolean boundingBoxIntersects, boolean expectedResult) {
        ...
        if (Objects.nonNull(boundingBoxIntersects)) {
            ...
            when(isochroneLabel.getEdge()).thenReturn(1);
            when(isochroneLabel.getNode()).thenReturn(2);
            ...
            assertThat(IsochroneFilter.isWithinBoundingBox(queryGraph, isochroneLabel, isochroneArguments)).isEqualTo(expectedResult);
        } else {
            assertThat(IsochroneFilter.isWithinBoundingBox(queryGraph, isochroneLabel, isochroneArguments)).isTrue();
        }
    }
    ...
    void isWithinMunicipality(Integer isochroneArgumentMunicipalityId, Integer encodedMunicipalityId, boolean expectedResult) {
        ...
        if (Objects.nonNull(isochroneArgumentMunicipalityId)) {
            ...
            assertThat(IsochroneFilter.isWithinMunicipality(encodingManager, queryGraph, isochroneLabel, isochroneArguments)).isEqualTo(
                    expectedResult);
        } else {
            assertThat(IsochroneFilter.isWithinMunicipality(encodingManager, queryGraph, isochroneLabel, isochroneArguments)).isTrue();
        }
    }
}
