...
class FuelTypeReasonJsonMapperTest {
    ...
    void mapToRestrictionJson() {
        when(fuelTypeReason.getValue()).thenReturn(Set.of(fuelType));
        when(fuelTypeMapper.map(fuelType)).thenReturn(fuelTypeJson);
        RestrictionJson actual = mapper.map(fuelTypeReason);
    ...
    @Test
    void mapperForType() {
        assertThat(mapper.mapperForType()).isEqualTo(ReasonType.FUEL_TYPE);
    }
}
