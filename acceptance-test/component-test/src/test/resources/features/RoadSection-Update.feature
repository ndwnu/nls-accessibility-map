Feature: Handle events to update road sections
  ...
  Scenario: update road section for a later nwb version will result in exception and when a new nwb version is available the retry mechanism can finally process the update
    Given a simple network with nwb version 2026-03-01
